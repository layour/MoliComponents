  ...
  _UModal.prototype = {
    ...
    _generateHTML: function() {
      ...
      var settings = this.settings,
          type = this.type,
          html,
          direction=settings.direction ? settings.direction:'';

      html = '<div class="um-modal '+direction+'"><div class="um-modal-content">';

      if (settings.title) {
        html += '<div class="um-modal-title '+direction+'">' + settings.title + '</div>';
      }
      if (settings.text) {
        html += '<div class="um-modal-text '+direction+'">';
      ...
      if (type === "toast") {
        ...
        html += '<a href="#" class="btn ok active">' + settings.btnText[1] +
            '</a></div></div>';
      }
    ...
    _showModal: function() {
      ...
      console.log(modal);
      modal.css({'marginTop':-modalH/ 2,'marginLeft':-modalW/2});
    ...
    destroy: function() {
     ...
     /* this.modal.removeClass('um-modal-in').addClass('um-modal-out').on('webkitTransitionEnd', function() {
      ...
      // 避免遮罩闪烁
      this.settings.overlay && $(this.modal).prev('.um-overlay').remove();
        this.modal.removeClass('um-modal-in').remove()
    },
    _init: function() {
